...
export default function AdminDashboard() {
  ...
  const navBtn = (target: typeof tab, label: string, Icon: any) => (
    <button onClick={() => setTab(target)}
      className={`w-full flex items-center gap-4 p-4 rounded-xl font-black uppercase tracking-widest text-[11px] transition-all ${
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans overflow-x-hidden">
      ...
      <aside className="fixed left-0 top-0 bottom-0 w-64 bg-white/[0.02] border-r border-white/5 p-8 flex flex-col z-50">
        <div className="mb-10">
          <h1 className="text-xl font-black uppercase tracking-tighter">Serendib <span className="text-[#D4AF37]">Admin</span></h1>
          <p className="text-[9px] font-black uppercase tracking-widest text-gray-500 mt-1">Fleet Dashboard</p>
        </div>
        <nav className="flex-1 space-y-2">
          <button onClick={() => navigate('/')}
            className="w-full flex items-center gap-4 p-4 rounded-xl font-black uppercase tracking-widest text-[10px] text-gray-500 hover:text-[#D4AF37] transition-all mb-4 border border-white/5 hover:border-[#D4AF37]/20">
        ...
        <button onClick={async () => { await signOut(); navigate('/admin/login'); }}
          className="p-4 flex items-center gap-4 text-red-500 font-black uppercase tracking-widest text-[10px] hover:bg-red-500/10 rounded-2xl transition-all">
      ...
      <main className="pl-64 p-10 max-w-[1600px]">
        ...
        {tab === 'inventory' && (
          <>
            ...
            <div className="space-y-3">
              {filtered.length === 0 ? (
                <div className="py-20 text-center text-xs font-black uppercase tracking-widest text-gray-600">No vehicles found</div>
              ) : filtered.map(v => {
                ...
                return (
                  <motion.div key={v.id} layout initial={{opacity:0}} animate={{opacity:1}}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <h4 className="text-lg font-black uppercase tracking-tighter truncate">{v.model}</h4>
                      <div className="flex items-center gap-4 mt-2 text-[10px] font-black uppercase tracking-widest text-gray-600">
                        <span className="text-white">LKR {(v.price/1_000_000).toFixed(1)}M</span>
        ...
        {tab === 'leads' && (
          <div className="space-y-10">
            <div>
              <h2 className="text-4xl font-black uppercase tracking-tighter">Acquisition <span className="text-gray-500">Leads</span></h2>
              <p className="text-[11px] font-black uppercase tracking-[0.2em] text-[#D4AF37] mt-1">{leads.length} Records</p>
            </div>
            <div className="space-y-4">
              {leads.length === 0 ? (
                <div className="py-20 text-center text-xs font-black uppercase tracking-widest text-gray-600">No leads yet</div>
              ) : leads.map(lead => (
                <div key={lead.id} className="p-6 bg-white/[0.02] border border-white/5 rounded-[28px] flex items-center gap-6">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-1">
                      <h4 className="text-lg font-black uppercase tracking-tighter">{lead.name}</h4>
        ...
        {tab === 'analytics' && (
          <div className="space-y-10 max-w-[1200px]">
            <div>
              <h2 className="text-4xl font-black uppercase tracking-tighter">Market <span className="text-gray-500">Pulse</span></h2>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="lg:col-span-8 p-8 bg-white/[0.02] border border-white/10 rounded-[32px]">
                <div className="flex items-center justify-between mb-8">
                  <h4 className="text-sm font-black uppercase tracking-widest text-[#D4AF37]">Top Interests (Clicks)</h4>
                  ...
                  {stats.topViewed.map(v => (
                    <div key={v.id} className="flex items-center gap-4 p-4 rounded-2xl hover:bg-white/5 transition-all group">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[9px] font-bold text-gray-500 uppercase tracking-widest">LKR {(v.price/1000000).toFixed(1)}M</p>
                      </div>
                      <div className="text-right">
                        <p className="text-lg font-black text-[#D4AF37] leading-none">{v.views || 0}</p>
                ...
                <div className="mt-12 pt-12 border-t border-white/5">
                  <div className="flex items-center justify-between mb-8">
                    <h4 className="text-sm font-black uppercase tracking-widest text-[#D4AF37]">Traffic Evolution</h4>
                    <span className="text-[10px] font-black uppercase tracking-widest text-gray-500">Last 7 Days</span>
                  </div>
                  <div className="flex items-end gap-3 h-32">
                    {traffic.length === 0 ? (
                      <div className="w-full h-full flex items-center justify-center text-[10px] font-black uppercase tracking-widest text-gray-600">Tracking started... waiting for data</div>
              ...
              <div className="lg:col-span-4 space-y-8">
                <div className="p-8 bg-white/[0.02] border border-white/10 rounded-[32px]">
                  <h4 className="text-sm font-black uppercase tracking-widest text-[#D4AF37] mb-8">Fleet Composition</h4>
                ...
                <div className="p-8 bg-[#D4AF37] text-black rounded-[32px] relative overflow-hidden group">
                  ...
                  <h4 className="text-xl font-black uppercase tracking-tighter mb-3 leading-tight relative z-10">Intelligent Tracking</h4>
